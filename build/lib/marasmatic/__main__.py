				...
				*itertools.islice(
					bases.Memory(
						Input(
							...
							encoding = frozenset[str](encoding)
						)
					).stream,
					length
				)
...
def bot(input: tuple[pathlib.Path], encoding: tuple[str], length: int, token: str, chat: str, interval: int, site: str, one: bool, pickled: pathlib.Path | None):
